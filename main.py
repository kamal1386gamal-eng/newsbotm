...
def process_caption_for_publishing(caption: str) -> str:
    """
    حذف تمام لینک‌ها (http, https, @mentions) و هشتگ‌ها (#hashtag)
    و اضافه کردن @spark_news_tel در انتها.
    """
...
async def send_to_channel(callback: types.CallbackQuery, user_data: Dict[str, Any]):
    ...
    try:
        if media_type == "photo":
            await bot.send_photo(CHANNEL_ID, media_data["file_id"], caption=caption)
        elif media_type == "video":
            await bot.send_video(CHANNEL_ID, media_data["file_id"], caption=caption)
        elif media_type == "audio":
            ...
            await bot.send_document(CHANNEL_ID, media_data["file_id"], caption=caption)
        elif media_type == "voice":
            await bot.send_voice(CHANNEL_ID, media_data["file_id"], caption=caption)
        elif media_type == "video_note":
